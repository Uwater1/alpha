...
def alpha_107(df: pd.DataFrame) -> pd.Series:
    ...
    rank_open_minus_delay_high = ts_rank(open_price - delay(high, 1), 20)
    
    # Calculate rank of open minus delay of close
    rank_open_minus_delay_close = ts_rank(open_price - delay(close, 1), 20)
    
    # Calculate rank of open minus delay of low
    rank_open_minus_delay_low = ts_rank(open_price - delay(low, 1), 20)
