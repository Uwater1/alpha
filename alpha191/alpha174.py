...
def alpha_174(df: pd.DataFrame) -> pd.Series:
    """
    Compute Alpha174 factor.
    ...
    return pd.Series(result, index=df.index, name='alpha_174')

def alpha174(code, benchmark='zz800', end_date="2026-01-23", lookback=350):
    return run_alpha_factor(alpha_174, code, benchmark, end_date, lookback)