...
def alpha_172(df: pd.DataFrame) -> pd.Series:
    """
    Compute Alpha172 factor.
    ...
    # Calculate MEAN(...,6)
    result = ts_mean(ratio, 6)
    
    return pd.Series(result, index=df.index, name='alpha_172')

def alpha172(code, benchmark='zz800', end_date="2026-01-23", lookback=350):
    return run_alpha_factor(alpha_172, code, benchmark, end_date, lookback)