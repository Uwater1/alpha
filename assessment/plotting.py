...
def plot_quantile_returns_bar(mean_ret: pd.DataFrame, ax: Optional[plt.Axes] = None, title: Optional[str] = None):
    """
    Plots mean returns by quantile as a bar chart.
    """
    if ax is None:
    ...
    mean_ret.plot(kind='bar', ax=ax, title=title)
...
def plot_ic_hist(ic_data: pd.DataFrame, ax: Optional[plt.Axes] = None):
    ...
    for col in ic_data.columns:
        sns.histplot(ic_data[col], ax=ax, kde=True, label=col, alpha=0.5)
